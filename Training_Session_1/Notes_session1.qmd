...
- In this training, we will strive to use the modern syntax `data %>% function()` or more generally 
`data %>% function(opt_arg = my_option)`
...
Let's consider the simple dataset below. 
This could be stored as rectangular table with 4 rows and 3 columns.
...
NB: writing explicitly `library(dataReporter)` loads all the package (functions and dataset). It is an 'overkill' when only one function is required. For parsimony (and to spare one line of code), we can skip `library(dataReporter)` and write
`PRRS %>% dataReporter::makeDataReport()`
...
- The exercise consists in writing a short R script that executes the various steps below:
  ...
  - rename the variable `wt` into `weight` using the function `rename()` of the R package `dplyr` 
  (hint: make sure the package is installed and loaded , type `? rename` for on-line help and see the example given)
